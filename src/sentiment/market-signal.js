...
export function combineMarketSignals(
  ...
  sentiment,
  {
     ...
     * 거시 층의 가중치입니다.
     *
     * **여기 기본값은 1로 둡니다.** 이 함수는 순수 계산이라 기본값이 "가중치를
     * 건드리지 않음"이어야 하고, 백테스트의 `--compare macro`·`macrotiming`이
     * 이 층을 재려면 곱해진 값이 필요하기 때문입니다. **운영의 결정은
     * `paper-runner.js`의 `readMacroWeight`에 0으로 박혀 있습니다.**
     *
