...
> **`LIVE_TRADING=true`로 두면 실제 주문이 나갑니다.** 실행기는 PAPER 장부를
> 그대로 돌린 뒤, 같은 결정을 실거래 사이클(`src/live/live-cycle.js`)에 넘깁니다.
> 두 장부를 나란히 돌려야 **체결 비용의 실측값**이 나오므로 PAPER는 꺼지지
> 않습니다.
>
> 켜기 전에 **보유 기준선을 한 번 기록해야 합니다** — `npm run live:baseline`.
> 없으면 계좌에 원래 있던 자산을 우리가 만든 것으로 오해하므로, 실행기가 매
> 사이클 대사에서 멈춥니다.
>
> 계좌가 여럿이면 `TOSS_ACCOUNT_SEQ`로 지정하십시오. 하나뿐이면 그것을 씁니다 —
> 무인 실행기가 어느 계좌인지 추측하지 않습니다.
>
> 멈추려면 `LIVE_TRADING=false`로 되돌리거나, 즉시 멈추려면 `npm run stop`.
